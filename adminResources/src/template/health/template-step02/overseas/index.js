$(function() {
    ...
    var CONFIG = {
        need_group_show: false, // 是否需要显示输入团体信息	
        need_LinkMan_ID: true // 是否需要输入联系人的身份信息
    };
    var DOM = {
        $loading: $('.loading'),
    ...
    var pageManager = {
        ...
        render: function(data) {
            ...
            if (data) { //填充缓存数据
                ...
                DOM.$linkManNamePY.val(data.linkManNamePY);
            ...
            if (productData) {
                ...
                $.ajax({
                    ...
                    fail: function() {
                        ...
                        window.wAlert('数据加载失败，请刷新页面');
                    },
                    error: function() {
                        DOM.$loading.hide();
                        console.log('reload failed');
                        window.wAlert('网络出错，请刷新页面');
                    }
                });
            }
        }
    ...
    function saveApplicantInfoToSessionStorage() {
        ...
        applicantShow.linkManNamePY = DOM.$linkManNamePY.val();

        //姓名拼音
        applicantShow.applicantNamePY = DOM.$linkManNamePY.val();
        ...
        if ($('#applicantCertType').val() === '01') {
            ...
            applicantShow.insuredBirthdayNew = $('#datePicker').val().trim();
            applicantShow.applicantBirthday = $('#datePicker').val().trim();
    ...
    DOM.$linkManName.on('blur', function() {
        var $name = DOM.$linkManName.val();
        var name = ConvertPinyin($name);

        DOM.$linkManNamePY.val(name);
    });
