...
function getDataFromSessionStorage() {
    ...
    $('#yourself input[name=insurantCertBirthday]').val($('#savebeinsured>input[name=applicantBirthday]').val());
    ...
    if (saveDatas) {
        ...
        for (var index = 0; index < length; index++) {
            if (index == 0) {
                ...
                $('#yourself input[name=insurantCertBirthday]').val(saveDatas[0].insurantCertBirthday);
...
function showAlert(m, n, setDate) {
    if (m == 1) {
        ...
        $("#commfirVpopWin").click(function() {
            ...
            var tr = $('<p>' + '<label class="insurantName_show left">' + $("#insurantNameVpopWin").val() + '</label>' + '<input type="hidden" class="insurantCertNo_show left" value="' + zjhaoma + '" readonly="readonly">' + '<input name="insurantName" type="hidden" value="' + $("#insurantNameVpopWin").val() + '" />' + '<input name="insurantCertType" type="hidden" value="' + $("#insurantCertTypeVpopWin").val() + '" />' + '<input name="insurantCertNo" type="hidden" value="' + zjhaoma + '" />' + '<input name="insurantCertBirthday" type="hidden" value="' + birthdaydate + '" />' + '<input name="insurantCertSex" type="hidden" value="' + sexdata + '" />' + '<input name="relationshipWithInsured" type="hidden" value="' + $('#relationshipWithInsuredVpopWin').val() + '" />' + '<span class="right tc" ><a href="javascript:;" onclick="changeThisRows($(this))" class="deleteThisRows">修改</a><a href="javascript:;" onclick="deleteThisRows($(this))" class="deleteThisRows">删除</a></span>' + '</p>')
...
function changeThisRows(obj) {
    ...
    $("#commfirVpopWin").click(function() {
        ...
        var current_index = obj.parent().parent().index()
        var cert = {};
        cert.certType = $('#insurantCertTypeVpopWin').val();
        if (cert.certType == '01') {
            cert.certNo = $("#for-paperNoVpopWin").val();
        ...
        if (!checkCertNoRepeatModify(cert, true, 'modify', current_index)) {
            return;
        }
...
function gotoPay() {
    ...
    if ($('#beInsuredList p').length < 1) {
        window.wAlert('被投保人不得少于1人');
        return false;
    } else if ($('#beInsuredList p').length > 50) {
        window.wAlert('被投保人不得多于五十人');
        return false;
    }
...
function checkCertNoRepeatModify(cert, isModified, type, currentIndex) {
    var certType = cert.certType;
    var certNo = cert.certNo;
    var certTypes = $('input[name=insurantCertType]');
    var certNos = $('input[name=insurantCertNo]');
