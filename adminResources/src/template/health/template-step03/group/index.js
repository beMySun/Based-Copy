...
function gotoPay() {
    ...
    if ($('#beInsuredList p').length < 3) {
        window.wAlert('被保险人不得少于三人');
        return false;
    } else if ($('#beInsuredList p').length > 50) {
        window.wAlert('被保险人不得多于五十人');
        return false;
    }
...
function valiateShenfenz(certNo, flag, showDiv) {
    ...
    if (certNo.length == 15) {
        ...
        var month = certNo.substring(8, 10);
        var day = certNo.substring(10, 12);
        var date1 = (parseInt(year, 10) + lessInsuranceAge) + "-" + month + "-" + day;
        var date3 = (parseInt(year, 10) + topInsuranceAge) + "-" + month + "-" + day;
        var d = new Date();
        var date2 = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate();
        if (dateCompare(date1, date2) || dateCompare(date2, date3)) {
            window.wAlert(description + "必须在" + lessInsuranceAge + "到" + topInsuranceAge + "周岁之间");
