...
//跟通用模板区别：多了目的地、是否办理签证
//动态报价source：travelSafety
//使用于模板：境外旅游任意门、境外旅游
...
(function() {
    ...
    var tool = {
        ...
        //选择目的地
        getDestination: function(url) {
            var html = window.sessionStorage.getItem('destinationHtml');
            if (html) {
                $('body').append(html);
            } else {
                $.ajax({
                    ...
                    success: function(data) {
                        window.sessionStorage.setItem('destinationHtml', data);
                        $('body').append(data);
                    },
                    error: function() {
                        window.wAlert('目的地加载失败，请刷新页面');
                    }
                });
            }
        },
    ...
    var pageCrl = {
        ...
        recoveryData: function(productData, submit) {
            ...
            //恢复签证和目的地【通用模板上新增】
            submit.myRadio === 'Y' && $('input[name="applyForVisa"]').eq(0).prop("checked", true);
            $('#selectCityDIV').val(submit.outgoingPurpose);
        ...
        init: function() {
                ...
                if (!submit) { //初次进入页面
                    ...
                    tool.getDestination('template/health/template-step01/travel/selectCity.html'); //【通用模板上新增】
                } else {
                ...
                $(document).on('click', '.close-picker', function(event) {
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            source: 'travelSafety' //不传就走默认的动态报价接口
                        },
                        success: function(data) {
                            ...
                            if (data && data.mesgCode === '0000') { //成功
                                ...
                                window.wAlert('动态报价失败，请重新选择保险起止日期');
                                pageCrl.initbeginDate(productData, planCode);
                                pageCrl.initEndDate(productData, planCode);
                ...
                //选择目的地的初始化时，若没加载成功，用户点击时再去加载【通用模板上新增】
                $('#selectCityDIV').on('click', function() {
                    if ($('.list-city').length === 0) {
                        tool.getDestination('template/health/template-step01/travel/selectCity.html');
                    }
                });
                ...
                $('#J-btn-buy').on('click', function() {
                    ...
                    var submit = {
                        ...
                        planCode: planCode + '',
                        myRadio: $('input[name="applyForVisa"]:checked').val(), //【通用模板上新增】
                        outgoingPurpose: $('#selectCityDIV').val() //【通用模板上新增】
                    };
                ...
                //end  event 点击购买
                $('head').addClass('close-picker').trigger('click').removeClass('close-picker');
