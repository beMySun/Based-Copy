...
//动态报价默认不带source，其他source值：travelSafety，ashInsuranceForGroup2为团意险个人投保
...
(function() {
    ...
    var pageCrl = {
        ...
        render: function(productData) {
            ...
            var html = '';
            for (var i = 0; i < planInfoList.length; i++) {
                var planList = planInfoList[i].remoteProMap.twoDimensionCodeTargetArray; //【通用模板上变量名有变动】
                ...
                if (planList && planList.length > 0) {
                    for (var j = 0; j < planList.length; j++) {
                        //begin 责任
                        var dutyList = planList[j].dutys; //【通用模板上变量名有变动】
                        if (dutyList && dutyList.length > 0) {
                            for (var k = 0; k < dutyList.length; k++) {
                                var tempHtml = [
                                    '<li>',
                                    '<span class="fn-left">',
                                    dutyList[k].dutyClauseName, //【通用模板上变量名有变动】
                                    '</span>',
                                    '<span>',
                                    dutyList[k].dutyInsuredAmount, //【通用模板上变量名有变动】
                                    '</span>',
                                    '<span>',
                                    '元',
                                    '</span>',
