;(function() {
	...
	var backRender = {
		backRender: function() {
			...
			if (pageData && payData) {
				var insurantList = payData.ahsPolicy.subjectInfo[0].subjectInfo.insurantInfo;
				$('#packageAmount').text(applicantData.packageAmount);
				$('#plateNumber').val(insurantList[0].insurantInfo.vehicleLicenceCode);
				$('#carframeNum').val(insurantList[0].insurantInfo.vehicleFrameNo);
				$('#motorNum').val(insurantList[0].insurantInfo.engineNo);
				...
				if (!submit.maxInsuredNumber || parseInt(submit.maxInsuredNumber) <= 1) {
					...
					if (applicantData.isSelf === '0') {
						var insurantInfo = insurantList[0].insurantInfo;
						$ulDom.find('.is-self').removeAttr('checked');
						$('.only-other-name').val(insurantInfo.personnelName);
					...
					for (var i=0; i<len; i++) {
						$($uls[i]).find('.select-certificate-type').val(insurantList[i].insurantInfo.relationshipWithApplicant);
						$($uls[i]).find('.to-validate-name').val(insurantList[i].insurantInfo.personnelName);
						$($uls[i]).find('.certificate-type').val(insurantList[i].insurantInfo.certificateType);
						$($uls[i]).find('.to-validate-certificate').val(insurantList[i].insurantInfo.certificateNo);
						if (insurantList[i].insurantInfo.certificateType !== '01') {
							backRender.getBirdthAndSexHtml($($uls[i]), insurantList[i].insurantInfo);
						}
						$($uls[i]).find('.insurance-phone').val(insurantList[i].insurantInfo.mobileTelephone);
						if (insurantList[i].insurantInfo.relationshipWithApplicant === '1') {
