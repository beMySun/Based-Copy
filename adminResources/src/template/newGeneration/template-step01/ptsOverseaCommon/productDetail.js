$(function() {
    ...
    //从产品中获取第一个套餐的套餐代码
    function getPackageCode(planCode) {
    ...
    function calculatePrice() {
        ...
        var packageCode = getPackageCode(planCode);
    ...
    var ProductDetail = {
        ...
        liabilityStruc: function(data) {
            var coverage = '';
            if (parseInt(data.insuranceCoverage) < 10000) {
                coverage += '<span class="money-icon">' + data.insuranceCoverage + '</span>';
            } else {
                coverage += '<span class="money-icon">' + data.insuranceCoverage / 10000 + '万</span>';
            };
            return '<li class="tc-detail-li">' + '<p class="left">' + (data.liabilityName || '') + '</p>' + '<p class="right">' + '' + coverage + '' + '<img src="img/btn_arrow_black_right@2x.png" alt="" class="icon-img">' + '</p>' + '</li>' + '<li  class="tc-text" style="display:none;">' + '<p>' + (data.liabilityDesc || '暂无描述') + '</p>' + '</li>';
        },
        ...
        initWechatShare: function() {
            ...
            var planCode, mediaSource;
            var shareUrl = window.location.href.split('#')[0];
            var currentPageUrl = window.location.href.split('#')[0];
            var shareTitle = $('header .title').text() || ' ';
            var shareImgUrl, shareDesc;
            var urlParams = getUrlParameters();
            // 保险商城分享的情况：
            // 图片： 酷保云logo; 文案：所在银行］客户经理  xxx ,竭诚为您服务
            // 参数中带来的信息：userId: 手机号， remark：张三｜宁波支行
            if (urlParams.userId && urlParams.remark) { // 从保险商城来
                var userName = urlParams.remark.split('|')[0];
                var bankName = urlParams.remark.split('|')[1];
                shareImgUrl = $("#bgImg")[0].src;
                shareDesc = bankName + '客户经理' + userName + ' 竭诚为您服务';
            } else {
                shareImgUrl = $("#bgImg")[0].src;
                shareDesc = productData.planInfoList[0].localProMap.planDesc;
            }
            ...
            var wechatShareData = {
                ...
                // 缓存数据，供后续流程使用
            sessionStorage.setItem('wechatShareData', JSON.stringify(wechatShareData));
        ...
        pay: function() {
            ...
            $("#pay").on('click', function() {
                if ($("#mask").css('display') == 'none') {
                    ...
                    submit.destination = $('#selectCityDIV').data('destination');
