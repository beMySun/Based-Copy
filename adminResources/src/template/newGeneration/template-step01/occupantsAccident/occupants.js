$(function() {
    ...
    var app = {
        ...
        changeProduct: function(planCode, planInfoList) {
            ...
            var productSchemeHtml = planInfoList.length < 2 ? '' : '<div class="product-scheme"><div class="form-group form-group-select"> <label> 选择产品：</label> <form><select class="form-control form-control-select" id="productScheme">' + productOptionHtml + '</select> </form> </div> </div>';
            ...
            if (packageList && packageList.length > 1) {
                $('.product-detail h2').css('border-top', '0');
            } else {
                $('.product-detail h2').css('border-top', '1px solid #ddd');
            }
        },
